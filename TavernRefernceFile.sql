...
INSERT INTO Rooms(Room_Id, Tavern_Id, Room_Status, Floor, Room_Rate)

        VALUES(1, 1, 'Occupied', 2, 40.00),
              (2, 1, 'Occupied', 2, 40.00),
              (1, 3, 'Occupied', 3, 160.00),
              (2, 3, 'Occupied', 2, 110.00),
              (3, 3, 'Occupied', 2, 110.00),
              (4, 3, 'Occupied', 3, 160.00);
...
INSERT INTO GuestRooms(Guest_Id, Date_Start, Date_End, Sale_Id, Room_Id)

        VALUES(5, '2008-01-21', '2008-01-29' , 3, 1),
              (2, '2008-03-10', '2008-03-12', 5, 2),
              (21, '2008-02-15', '2008-02-17', 4, 1),
              (23, '2008-01-21', '2008-01-23', 9, 2),
              (18, '2008-02-15', '2008-02-20', 10, 3),
              (14, '2008-03-10', '2008-03-12', 11, 4);
...
/*#4. Query that returns the top 10 sales, with price and service description.*/
    ...
    SELECT Top 10 sales.Price, services.Service_Name FROM sales RIGHT JOIN services ON sales.Service_Id = services.Service_Id; 
    SELECT Top 10 sales.Price, sales.Guest_Id, services.Service_Name FROM sales LEFT JOIN services ON sales.Service_Id = services.Service_Id; 
...
/*#5. Query that returns guests with 2 or more classes. */
      ...
      SELECT GuestClasses.Guest_Id, COUNT(*) FROM GuestClasses GROUP BY Guest_Id;
        ...
        SELECT Guests.Guest_Name, GuestClasses.Guest_Id, COUNT(*) FROM GuestClasses LEFT JOIN Guests ON GuestClasses.Guest_Id = Guests.Guest_Id GROUP BY Guest_Id HAVING COUNT(*) >= 2;
...
/*#8. Query that returns a guests stay in a room.*/ 
        ...
        SELECT GuestRooms.Guest_Id, GuestRooms.Date_Start, GuestRooms.Date_End, DATEDIFF(day, GuestRooms.Date_End, GuestRooms.Date_Start) FROM GuestRooms INNER JOIN Guests ON Guests.Guest_Id = GuestRooms.Guest_Id; 
