...
/*#9. Query that makes a returned table which shows a key constraint associated with the column name. */ 
        --I tried for a whole hour but my inner JOIN could've work, I bieleve a CASE may do the trick, I will try again. 
        
        SELECT CONCAT('CREATE TABLE ', TABLE_NAME, ' (' ) as CreateNewTable FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'Taverns'
UNION SELECT CONCAT('Key ', CONSTRAINT_TYPE ) as Constraints FROM INFORMATION_SCHEMA.TABLE_CONSTRAINTS WHERE CONSTRAINT_TYPE = 'FOREIGN KEY'

UNION SELECT INFORMATION_SCHEMA.KEY_COLUMN_USAGE.TABLE_NAME FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE INNER JOIN INFORMATION_SCHEMA.COLUMNS 
ON INFORMATION_SCHEMA.KEY_COLUMN_USAGE.TABLE_NAME = INFORMATION_SCHEMA.COLUMNS.COLUMN_NAME 

INNER JOIN INFORMATION_SCHEMA.TABLE_CONSTRAINTS.CONSTRAINT_TYPE ON INFORMATION_SCHEMA.COLUMNS.COLUMN_NAME = INFORMATION_SCHEMA.TABLE_CONSTRAINTS.TABLE_NAME
...
LEFT JOIN (SELECT DISTINCT CONSTRAINT_NAME, TABLE_NAME, COLUMN_NAME FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE*/
